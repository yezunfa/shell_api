...
function checkOrderResponse(result)
{
  ...
  try
  {
	  var total = parseInt(result.new_orders)+parseInt(result.new_paid)+parseInt(result.user_account)+parseInt(result.shop_account)+parseInt(result.no_paid)+parseInt(result.no_change)+parseInt(result.wait_cash)+parseInt(result.advance_date)+parseInt(result.goods_report)+parseInt(result.complaint);
	 $("#total").html(total);
