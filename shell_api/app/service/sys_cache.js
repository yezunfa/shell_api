...
class SysCacheService extends Service {
    ...
    /**
     * create a new sys_cache
     * @param {Object} entity model sys_cache
     * @return {Object} entity a model Entity
     */
    async create(entity) {
        try {
            const result = await this.ctx.model.SysCache.create(entity);
            return result;
        } catch (err) {
            throw err;
        }
    ...
    /**
     *  get a sys_cache by Id
     * @param {String} Id guid
     * @return {Object} entity a model Entity
     */
    async getById(Id) {
        try {
            const result = await this.ctx.model.SysCache.findOne({
                where: {
                    Id
                }
            });
    ...
    /**
     * getLatest record from sys_cache
     */
    async getLatest() {
        const result = await this.ctx.model.SysCache.findOne({
    ...
    async edit(entity) {
        try {
            const result = await this.ctx.model.SysCache.update(entity.dataValues || entity, {
                where: {
                    Id: entity.Id
                }
            });
    ...
    /**
     * remove a record from sys_cache
    ...
    async remove(entity) {
        try{
            entity.Valid = 0;
            return await this.ctx.service.sysCache.edit(entity);
        } catch (err) {
            throw err;
        }
    }
    ...
    /**
     * delete a record from sys_cache
     * @param {Object} entity a model Entity
     * @return {Object} affact count
     */
    async delete(entity) {
        try {
            const result = await this.ctx.model.SysCache.destroy({
                where: {
                    Id: entity.Id
                }
            });
    ...
    /**
     * search in SysCache and left jion with {  }
    ...
    async search(pagination, where, order) {
        ...
        const querySql = (isCount) => {
            ...
            let sql = "";
            if(isCount) {
                sql = sqlHelper.buildWhereCondition(whereCon, 'sys_cache');
            } else {
                selectVal = "sys_cache.*";
                sql = sqlHelper.buildCondition(pagination, whereCon, orderCon, 'sys_cache');
            }
