...
const autoCode = require('./code-auto');
  ...
  run(callback) {
    ...
    function generateText(err) {
      ...
      async.each(_.keys(self.tables), function(table, _callback) {
        ...
        var tableFieldComments =  self.tableFieldComments[table];
        ...
        _.each(fields, function(field, i) {
          ...
          // has comments
          try{
            if(tableFieldComments[field]) {
           
              text[table] += spaces + spaces + spaces + "comment: '" + tableFieldComments[field] + "',\n";
            }
          } catch(ex) {
            console.log(table,field, tableFieldComments)
          }
        ...
        try{
          ...
          console.log(ex,'auto code exception');
        }
