...
class CompanyService extends Service {
     ...
     * @param {Object} entity model company
     * @return {Object} entity a model Entity
     */
    async create(entity) {
        try {
            const result = await this.ctx.model.Company.create(entity);
            return result;
        } catch (err) {
            throw err;
        }
    ...
    async getById(Id) {
        try {
            const result = await this.ctx.model.Company.findOne({
                where: {
                    Id
                }
            });
    ...
    async edit(entity) {
        try {
            const result = await this.ctx.model.Company.update(entity.dataValues || entity, {
                where: {
                    Id: entity.Id
                }
            });
    ...
    async remove(entity) {
        try{
            entity.Valid = 0;
            return await this.ctx.service.company.edit(entity);
        } catch (err) {
            throw err;
        }
    }
    ...
    async delete(entity) {
        try {
            const result = await this.ctx.model.Company.destroy({
                where: {
                    Id: entity.Id
                }
            });
    ...
    /**
     * search in Company and left jion with {  }
    ...
    async search(pagination, where, order) {
        ...
        const querySql = (isCount) => {
            ...
            let sql = "";
            if(isCount) {
                sql = sqlHelper.buildWhereCondition(whereCon, 'company');
            } else {
                selectVal = "company.*";
                sql = sqlHelper.buildCondition(pagination, whereCon, orderCon, 'company');
            }
