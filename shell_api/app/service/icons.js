...
class IconsService extends Service {
    ...
    /**
     * create a new icons
     * @param {Object} entity model icons
     * @return {Object} entity a model Entity
     */
    async create(entity) {
        try {
            const result = await this.ctx.model.Icons.create(entity);
            return result;
        } catch (err) {
            throw err;
        }
    ...
    async getById(Id) {
        try {
            const result = await this.ctx.model.Icons.findOne({
                where: {
                    Id
                }
            });
    ...
    async edit(entity) {
        try {
            const result = await this.ctx.model.Icons.update(entity.dataValues || entity, {
                where: {
                    Id: entity.Id
                }
            });
    ...
    async remove(entity) {
        try{
            entity.Valid = 0;
            return await this.ctx.service.icons.edit(entity);
        } catch (err) {
            throw err;
        }
    }
    ...
    /**
     * delete a record from icons
     * @param {Object} entity a model Entity
     * @return {Object} affact count
     */
    async delete(entity) {
        try {
            const result = await this.ctx.model.Icons.destroy({
                where: {
                    Id: entity.Id
                }
            });
    ...
    /**
     * search in Icons and left jion with {  }
    ...
    async search(pagination, where, order) {
        ...
        const querySql = (isCount) => {
            ...
            let sql = "";
            if(isCount) {
                sql = sqlHelper.buildWhereCondition(whereCon, 'icons');
            } else {
                selectVal = "icons.*";
                sql = sqlHelper.buildCondition(pagination, whereCon, orderCon, 'icons');
            }
