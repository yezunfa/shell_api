...
class TestService extends Service {
    ...
    /**
     * create a new test
     * @param {Object} entity model test
     * @return {Object} entity a model Entity
     */
    async create(entity) {
        try {
            const result = await this.ctx.model.Test.create(entity);
            return result;
        } catch (err) {
            throw err;
        }
    ...
    async getById(Id) {
        try {
            const result = await this.ctx.model.Test.findOne({
                where: {
                    Id
                }
            });
    ...
    async edit(entity) {
        try {
            const result = await this.ctx.model.Test.update(entity.dataValues || entity, {
                where: {
                    Id: entity.Id
                }
            });
    ...
    async remove(entity) {
        try{
            entity.Valid = 0;
            return await this.ctx.service.test.edit(entity);
        } catch (err) {
            throw err;
        }
    }
    ...
    /**
     * delete a record from test
     * @param {Object} entity a model Entity
     * @return {Object} affact count
     */
    async delete(entity) {
        try {
            const result = await this.ctx.model.Test.destroy({
                where: {
                    Id: entity.Id
                }
            });
    ...
    /**
     * search in Test and left jion with {  }
    ...
    async search(pagination, where, order) {
        ...
        const querySql = (isCount) => {
            ...
            let sql = "";
            if(isCount) {
                sql = sqlHelper.buildWhereCondition(whereCon, 'test');
            } else {
                selectVal = "test.*";
                sql = sqlHelper.buildCondition(pagination, whereCon, orderCon, 'test');
            }
