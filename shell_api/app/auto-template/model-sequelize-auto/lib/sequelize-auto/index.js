...
AutoSequelize.prototype.build = function(callback) {
  ...
  function mapForeignKeys(table, fn) {
    ...
    var commentSql = self.dialect.getTableFiledComment ? self.dialect.getTableFiledComment(table, self.sequelize.config.database) : null;
    self.sequelize.query(sql, {
      type: self.sequelize.QueryTypes.SELECT,
      raw: true
    }).then(function (res) {
      if(commentSql) {
        self.sequelize.query(commentSql, {
          type: self.sequelize.QueryTypes.SELECT,
          raw: true
        }).then(function (resTableFieldComents) {
         
          async.each(resTableFieldComents, assignCommentsDetails)
    ...
    function assignCommentsDetails(ref) {
      self.tableFieldComments = self.tableFieldComments || {};
      self.tableFieldComments[table] = self.tableFieldComments[table] || {};
      self.tableFieldComments[table][ref.COLUMN_NAME] = ref.column_comment;
    }
...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      text[table] = "'use strict'; \n/* jshint indent: " + self.options.indentation + " */\n\n";
