...
class TaskService extends Service {
    ...
    /**
     * create a new task
     * @param {Object} entity model task
     * @return {Object} entity a model Entity
     */
    async create(entity) {
        try {
            const result = await this.ctx.model.Task.create(entity);
            return result;
        } catch (err) {
            throw err;
        }
    ...
    async getById(Id) {
        try {
            const result = await this.ctx.model.Task.findOne({
                where: {
                    Id
                }
            });
    ...
    async edit(entity) {
        try {
            const result = await this.ctx.model.Task.update(entity.dataValues || entity, {
                where: {
                    Id: entity.Id
                }
            });
    ...
    async remove(entity) {
        try{
            entity.Valid = 0;
            return await this.ctx.service.task.edit(entity);
        } catch (err) {
            throw err;
        }
    }
    ...
    /**
     * delete a record from task
     * @param {Object} entity a model Entity
     * @return {Object} affact count
     */
    async delete(entity) {
        try {
            const result = await this.ctx.model.Task.destroy({
                where: {
                    Id: entity.Id
                }
            });
    ...
    /**
     * search in Task and left jion with {  }
    ...
    async search(pagination, where, order) {
        ...
        const querySql = (isCount) => {
            ...
            let sql = "";
            if(isCount) {
                sql = sqlHelper.buildWhereCondition(whereCon, 'task');
            } else {
                selectVal = "task.*";
                sql = sqlHelper.buildCondition(pagination, whereCon, orderCon, 'task');
            }
