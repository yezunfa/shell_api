...
class TaskTypeService extends Service {
     ...
     * @param {Object} entity model task_type
     * @return {Object} entity a model Entity
     */
    async create(entity) {
        try {
            const result = await this.ctx.model.TaskType.create(entity);
            return result;
        } catch (err) {
            throw err;
        }
    ...
    /**
     *  get a task_type by Id
     * @param {String} Id guid
     * @return {Object} entity a model Entity
     */
    async getById(Id) {
        try {
            const result = await this.ctx.model.TaskType.findOne({
                where: {
                    Id
                }
            });
    ...
    /**
     * getLatest record from task_type
     */
    async getLatest() {
        const result = await this.ctx.model.TaskType.findOne({
    ...
    async edit(entity) {
        try {
            const result = await this.ctx.model.TaskType.update(entity.dataValues || entity, {
                where: {
                    Id: entity.Id
                }
            });
    ...
    /**
     * remove a record from task_type
    ...
    async remove(entity) {
        try{
            entity.Valid = 0;
            return await this.ctx.service.taskType.edit(entity);
        } catch (err) {
            throw err;
        }
    }
    ...
    /**
     * delete a record from task_type
     * @param {Object} entity a model Entity
     * @return {Object} affact count
     */
    async delete(entity) {
        try {
            const result = await this.ctx.model.TaskType.destroy({
                where: {
                    Id: entity.Id
                }
            });
    ...
    /**
     * search in TaskType and left jion with {  }
    ...
    async search(pagination, where, order) {
        ...
        const querySql = (isCount) => {
            ...
            let sql = "";
            if(isCount) {
                sql = sqlHelper.buildWhereCondition(whereCon, 'task_type');
            } else {
                selectVal = "task_type.*";
                sql = sqlHelper.buildCondition(pagination, whereCon, orderCon, 'task_type');
            }
