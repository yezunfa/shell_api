...
class StoreService extends Service {
    ...
    /**
     * create a new store
     * @param {Object} entity model store
     * @return {Object} entity a model Entity
     */
    async create(entity) {
        try {
            const result = await this.ctx.model.Store.create(entity);
            return result;
        } catch (err) {
            throw err;
        }
    ...
    async getById(Id) {
        try {
            const result = await this.ctx.model.Store.findOne({
                where: {
                    Id
                }
            });
    ...
    async edit(entity) {
        try {
            const result = await this.ctx.model.Store.update(entity.dataValues || entity, {
                where: {
                    Id: entity.Id
                }
            });
    ...
    async remove(entity) {
        try{
            entity.Valid = 0;
            return await this.ctx.service.store.edit(entity);
        } catch (err) {
            throw err;
        }
    }
    ...
    async delete(entity) {
        try {
            const result = await this.ctx.model.Store.destroy({
                where: {
                    Id: entity.Id
                }
            });
    ...
    /**
     * search in Store and left jion with {  }
    ...
    async search(pagination, where, order) {
        ...
        const querySql = (isCount) => {
            ...
            let sql = "";
            if(isCount) {
                sql = sqlHelper.buildWhereCondition(whereCon, 'store');
            } else {
                selectVal = "store.*";
                sql = sqlHelper.buildCondition(pagination, whereCon, orderCon, 'store');
            }
